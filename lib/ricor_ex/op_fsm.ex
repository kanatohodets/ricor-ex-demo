defmodule RicorEx.OpFSM do
  ...
  def op(op, key, n, w) do
	req_id = req_id()
	RicorEx.OpFSM.Sup.start_op_fsm([req_id, self(), op, key, n, w])
	{:ok, req_id}
  end
  ...
  def prepare(:timeout, %{n: n, key: key}=state_data) do
    ...
    pref_list = :riak_core_apl.get_apl(doc_idx, n, RicorEx.Service)
  ...
  def execute(:timeout, %{req_id: req_id, op: op, preflist: pref_list}=state_data) do
    command = {req_id, op}
    :riak_core_vnode_master.command(pref_list, command, {:fsm, :undefined, self()}, RicorEx.Vnode_master)
